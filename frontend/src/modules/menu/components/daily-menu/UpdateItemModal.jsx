...
const parseFormValues = (values) => {
  ...
  if (parsed.preparedQuantity === '' || parsed.preparedQuantity === null || parsed.preparedQuantity === undefined) {
    parsed.preparedQuantity = undefined;
  } else {
    parsed.preparedQuantity = Number(parsed.preparedQuantity);
  }

  if (parsed.currentPrice === '' || parsed.currentPrice === null || parsed.currentPrice === undefined) {
...
const makeResolver = (item) => (values) => {
  ...
  const refinedSchema = updateItemSchema.superRefine((data, ctx) => {
    if (data.preparedQuantity !== undefined && data.preparedQuantity < (item?.soldQuantity || 0)) {
      ctx.addIssue({
        code: 'custom',
...
const UpdateItemModal = ({ open, item, onSubmit, onClose, isLoading }) => {
  ...
  const onFormSubmit = (data) => {
    ...
    if (data.preparedQuantity !== undefined && data.preparedQuantity !== item.preparedQuantity) {
      payload.preparedQuantity = data.preparedQuantity;
    }

    if (data.currentPrice !== undefined && data.currentPrice !== item.currentPrice) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative bg-surface-container-lowest rounded-2xl shadow-elevated p-6 w-full max-w-md mx-4 animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="text-headline-sm font-bold text-on-surface">Cập nhật món</h3>
        ...
        <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4">
          ...
          <div>
            ...
            {errors.preparedQuantity && (
              <p className="text-[11px] text-error mt-1">{errors.preparedQuantity.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            />
            {errors.currentPrice && (
              <p className="text-[11px] text-error mt-1">{errors.currentPrice.message}</p>
            )}
          </div>

          {/* Reason (shown when price changed) */}
          {watchedPrice !== '' && watchedPrice !== undefined && Number(watchedPrice) !== item.currentPrice && (
            <div>
              <label className="block text-label-md text-on-surface-variant mb-1.5">
                Lý do thay đổi giá
              </label>
          ...
          <div>
            ...
            <select
              ...
              {...register('status')}
            >
              <option value={DAILY_MENU_ITEM_STATUS.AVAILABLE}>Còn phục vụ</option>
              <option value={DAILY_MENU_ITEM_STATUS.UNAVAILABLE}>Hết / Ngừng</option>
            </select>
          </div>
