...
const getOrderReferenceDate = (order) => order?.orderedAt || order?.createdAt || null;
...
const initialState = {
  cart: {
    items: [], // Array of { foodItemId, name, unitPrice, quantity, note }
  },
...
const orderSlice = createSlice({
  ...
  reducers: {
    addToCart(state, action) {
      const { foodItemId, name, unitPrice, quantity = 1, note = null } = action.payload;
